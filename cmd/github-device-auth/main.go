...
func run(ctx context.Context, args []string) error {
	...
	switch args[0] {
	case "device":
		if len(args) != 2 { return fmt.Errorf("usage: github-device-auth device <client-id>") }
		value, err = deviceauth.RequestDeviceCode(ctx, args[1])
	case "poll":
		if len(args) != 3 { return fmt.Errorf("usage: github-device-auth poll <client-id> <device-code>") }
		value, err = deviceauth.PollForToken(ctx, args[1], args[2], 5*time.Second)
	case "refresh":
		if len(args) != 3 && len(args) != 4 { return fmt.Errorf("usage: github-device-auth refresh <client-id> <refresh-token> [client-secret]") }
		secret := ""
		if len(args) == 4 { secret = args[3] }
		value, err = deviceauth.RefreshAccessToken(ctx, args[1], secret, args[2])
	case "login": return login(ctx, args[1:])
	case "import-env": return importEnv(args[1:])
	case "status": return status(args[1:])
	case "ensure": return ensure(ctx, args[1:], false)
	case "refresh-local": return ensure(ctx, args[1:], true)
	case "token": return token(args[1:])
	case "export": return exportBundle(args[1:])
	case "sync-secret": return syncSecret(ctx, args[1:])
	default: return fmt.Errorf("unknown command %q", args[0])
	}
...
func login(ctx context.Context, args []string) error {
	if len(args) != 2 { return fmt.Errorf("usage: github-device-auth login <client-id> <store>") }
	...
	if err != nil { return err }
	if err := deviceauth.SaveCredentialBundle(store, bundle); err != nil { return err }
	return printStatus(bundle, false)
}

func importEnv(args []string) error {
	if len(args) != 2 { return fmt.Errorf("usage: github-device-auth import-env <environment-variable> <store>") }
	value := os.Getenv(args[0])
	if value == "" { return fmt.Errorf("environment variable %s is empty", args[0]) }
...
func status(args []string) error {
	if len(args) != 1 { return fmt.Errorf("usage: github-device-auth status <store>") }
...
func parseStoreMargin(args []string) (string, time.Duration, error) {
	if len(args) != 1 && len(args) != 2 { return "", 0, fmt.Errorf("usage: github-device-auth <ensure|refresh-local> <store> [safety-margin-seconds]") }
	margin := defaultSafetyMargin
	if len(args) == 2 {
		seconds, err := strconv.Atoi(args[1])
		if err != nil || seconds < 0 { return "", 0, fmt.Errorf("invalid safety margin %q", args[1]) }
...
func token(args []string) error {
	if len(args) != 1 { return fmt.Errorf("usage: github-device-auth token <store>") }
	bundle, err := deviceauth.LoadCredentialBundle(args[0])
	if err != nil { return err }
	if !bundle.AccessFresh(time.Now(), 0) { return fmt.Errorf("access token is expired; run ensure first") }
	fmt.Fprintln(os.Stdout, bundle.AccessToken)
	return nil
}

func exportBundle(args []string) error {
	if len(args) != 1 { return fmt.Errorf("usage: github-device-auth export <store>") }
	bundle, err := deviceauth.LoadCredentialBundle(args[0])
	if err != nil { return err }
...
// when supplied, and only then replaces the primary repository secret. This is
// important for rotating GitHub refresh tokens: using a refresh token
// invalidates the old access+refresh pair immediately, so the freshly rotated
// pair needs durable recovery material before primary cutover.
func syncSecret(ctx context.Context, args []string) error {
	...
	if err != nil { return fmt.Errorf("sync-secret requires gh CLI: %w", err) }
	...
	if len(args) == 4 {
		if strings.TrimSpace(args[3]) == "" || args[3] == args[2] { return fmt.Errorf("recovery secret must be non-empty and distinct from primary") }
		if err := set(args[3]); err != nil { return fmt.Errorf("write recovery credential checkpoint: %w", err) }
	}
	if err := set(args[2]); err != nil { return fmt.Errorf("write primary credential checkpoint: %w", err) }
	return nil
}
